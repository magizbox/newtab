...
CalendarManager.extractEvent = function(elem, mailId) {
  ...
  /// Addition
  out.titleLong = out.title;
  var sliceStart = out.isAllDay ? 3 : (isDateToday(out.startTime) ? 1 : 4);
  out.title = out.title.split(' ').slice(sliceStart).join(' ');
...
function isDateToday(date) {
  var now = new Date, delta = (now - date), HOURS = 60*60*1000;
  return (Math.abs(delta) < 24*HOURS && now.getDate() == date.getDate());
}
