  ...
  function factory($) {
    ...
    Crop.prototype = {
      init: function () {
        ...
        if ($.fn.mousewheel) {
          this.$frame.on('mousewheel.' + pluginName, function (e) {
            e.preventDefault();
            if (e.deltaY < 0)
              self.zoomIn.call(self);
            else
      ...
      updateOptions: function () {
        ...
        img.onload = function () {
          ...
          if (self.options.result)
            ...
            self.zoom.call(self, self.fitPercent);///changed minPercent -> fitPercent
      ...
      fit: function () {
        ...
        this.fitPercent = Math.max(widthRatio, heightRatio);/// added fit percent
        this.minPercent = Math.min(widthRatio, heightRatio) / 2;///changed min percent
      },
...
*/
        this.img_left = (this.options.width - this.img_width) / 2;
        this.img_top = (this.options.height - this.img_height) / 2;
        this.$image.css({ width: this.img_width, left: this.img_left, top: this.img_top });
        this.update();
      },
      zoomIn: function() {
      ...
      drag: function(data, skipupdate) {
        this.img_left = data.startX + data.dx
        ///fill(data.startX + data.dx, this.img_width, this.options.width);
        this.img_top = data.startY + data.dy
        ///fill(data.startY + data.dy, this.img_height, this.options.height);
      ...
      getDataURL: function () {
        ...
        var pixelRatio = window.devicePixelRatio || 1;/// added pixelRatio
